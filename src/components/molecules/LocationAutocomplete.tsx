...
export const LocationAutocomplete: React.FC<LocationAutocompleteProps> = ({
  ...
  useEffect(() => {
    if (
      ...
      storeLocation.address !== inputValue &&
      currentSelectedTarget === target
    ) {
      handleInputChange(storeLocation.address);
    }
  }, [
    storeLocation,
    inputValue,
    ...
    >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-(--border) bg-(--bg-card) py-2.5 pr-10 pl-10 text-sm text-(--text-main) placeholder:text-(--text-muted) transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-(--primary) focus:ring-offset-2"
        />
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div
            ...
            role="listbox"
            className="absolute z-50 mt-1 max-h-72 w-full overflow-y-auto rounded-xl border border-(--border) bg-(--bg-card) shadow-lg"
          >
